...
def convert(dataset_root, detections_root, detector, save_path, split):
    datset_split = split
    all_seqs = len(
        os.listdir(os.path.join(dataset_root, datset_split, 'velodyne')))
    ...
    for seq_id in tqdm(range(all_seqs),
                       desc=f'Convert dataset to MCTrack format'):
        ...
        scene_datas = []
        for frame_id in dets_list:
            frame_name = f"{frame_id:06d}.txt"
            lidar2global = ego_poses[frame_id]

            cameras_transform_matrix = load_camera_info(
                lidar2camera, camera2image)
            det_path = os.path.join(dets_path, frame_name)
            bboxes = convert_baseversion_bboxes(det_path, lidar2global,
                                                lidar2camera)
